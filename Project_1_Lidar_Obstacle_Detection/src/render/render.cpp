...
// helper function that renders a box at the van location
void renderVanBox(pcl::visualization::PCLVisualizer::Ptr& viewer, int clusterId, float xmin, float ymin, float zmin, float xmax, float ymax, float zmax, bool display) {
    ...
    if (display) {
        ...
        box.y_max = ymax;
        box.z_max = zmax;
        renderBox(viewer, box, clusterId, Color(1, 0, 1), 0.7);
    }
}
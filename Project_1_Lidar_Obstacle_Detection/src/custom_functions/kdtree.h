// 3D implementation of KdTree3D by Sue Nopachinda
...
// processes 3D points - takes into account the Z axis in the point cloud
struct KdTree
{
	Node* root;
	...
	void insertHelper(Node *&node, uint depth, std::vector<float> point, int id) {
		...
		if (node == NULL) {
			...
			uint dimension = depth % 3;	// even = 0; odd == 1
	...
	void searchHelper(std::vector<float> target, Node* node, int depth, float distanceTol, std::vector<int>& ids) {
		if (node != NULL) {
			...
			float minZ = target[2] - distanceTol;
			float maxZ = target[2] + distanceTol;

			// check if the point is in the box boundaries
			if (node->point[0] >= minX && node->point[0] <= maxX && node->point[1] >= minY && node->point[1] <= maxY && node->point[2] >= minZ && node->point[2] <= maxZ) {
				// d = sqrt( a*2 + b^2 + c^2)
				float a_2 = ((node->point[0] - target[0]) * (node->point[0] - target[0]));
				float b_2 = ((node->point[1] - target[1]) * (node->point[1] - target[1]));
				float c_2 = ((node->point[2] - target[2]) * (node->point[2] - target[2]));
				float distance = sqrt( a_2 + b_2 + c_2 );
			...
			int boundary = depth % 3;	// x, y, z point index
